...
public class BlockManager
{
	 ...
	 */
	private static Semaphore s1 = new Semaphore(-9); // -9 requires 10 V() operations before any 2nd phase for any thread can start 

	/**
	 * s2 is for use in conjunction with Thread.turnTestAndSet() for phase II proceed
	 * in the thread creation order
	 */
	private static Semaphore s2 = new Semaphore(1); // Uncomment the new semaphore
	...
	static class AcquireBlock extends BaseThread
	{
		...
		public void run()
		{
			...
			s1.V(); 
			s1.P(); 
			s1.V(); // Release the semaphore when phase 2 is complete
			s2.P(); // From then, only one thread can enter phase 2 at a time
			while (!turnTestAndSet())
			{
				s2.V(); 
				s2.P(); 
				// These two lines give other threads a chance to check their turn
			}
			phase2();
	...
	static class CharStackProber extends BaseThread
	{
		public void run()
		{
			...
			s1.V(); 
			s1.P(); 
			s1.V(); // Make sure phase 1 is done then signal each to go
			s2.P(); // From then, only one thread can enter phase 2 at a time
			while (!turnTestAndSet())
			{
				s2.V(); 
